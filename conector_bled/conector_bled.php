...
class conector_bled extends Module{
	...
	public function hookActionUpdateQuantity($params) {
		try {
	        $this->escribirLog('Se actualizó el stock');
			if ( $this->getStatus() ) {
				try {
			...
			$this->escribirLog('No se actualizó el stock');
		}
	}

	public function hookActionProductSave($params) {
		try {
	        $this->escribirLog('Se guardó el producto');
			if ( $this->getStatus() ) {
			...
			$this->escribirLog('No se guardó el producto');
		}
	}

	public function hookActionFeatureSave($params) {
		try {
	        $this->escribirLog('Se guardó la caracteristica');
			if ( $this->getStatus() ) {
				$feature = Feature::getFeature((int)Configuration::get('conector_lang'), $params['id_feature']);
				$this->actualizarCaracteristica($feature);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se guardó la caracteristica');
		}
	}

	public function hookActionFeatureValueSave($params) {
		try {
	        $this->escribirLog('Se guardó el valor de la caracteristica');
			if ( $this->getStatus() ) {
				$sql = 'SELECT `id_feature` FROM `'._DB_PREFIX_.'feature`
						WHERE `id_lang`='.(int)Configuration::get('conector_lang').'
				...
				$feature_value = array(
							...
							'value' => FeatureValue::selectLang(FeatureValue::getFeatureValueLang($params['id_feature_value']),(int)Configuration::get('conector_lang'))
						);
				$this->actualizarValorCaracteristica($feature_value);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se guardó el valor de la caracteristica');
		}
	}

	public function hookActionAttributeGroupSave($params) {
		try {
	        $this->escribirLog('Se guardó el atributo');
			if ( $this->getStatus() ) {
	...
	public function hookActionAttributeSave($params) {
		try {
	        $this->escribirLog('Se guardó el valor del atributo');
			if ( $this->getStatus() ) {
			...
			$this->escribirLog('No se guardó el valor del atributo');
		}
	}

	public function hookActionProductDelete($params) {
		try {
	        $this->escribirLog('Se borró el producto');
			if ( $this->getStatus() ) {
				$this->borrar_producto($params['id_product']);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se borró el producto');
		}
	}

	public function hookActionFeatureDelete($params) {
		try {
	        $this->escribirLog('Se borró la caracteristica');
			if ( $this->getStatus() ) {
				$this->borrar_caracteristica($params['id_feature']);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se borró la caracteristica');
		}
	}

	public function hookActionFeatureValueDelete($params) {
		try {
	        $this->escribirLog('Se borró el valor de la caracteristica');
			if ( $this->getStatus() ) {
				$this->borrar_valor_caracteristica($params['id_feature_value']);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se borró el valor de la caracteristica');
		}
	}

	public function hookActionAttributeGroupDelete($params) {
		try {
	        $this->escribirLog('Se borró el atributo');
			if ( $this->getStatus() ) {
				$this->borrar_atributo($params['id_attribute_group']);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se borró el atributo');
		}
	}

	public function hookActionAttributeDelete($params) {
		try {
	        $this->escribirLog('Se borró el valor del atributo');
			if ( $this->getStatus() ) {
				$this->borrar_valor_atributo($params['id_attribute']);
			}
		} catch ( Exception $e ) {
			$this->escribirLog('No se borró el valor del atributo');
		}
	}
	...
	public function borrar_atributo($id_atributo) {
		...
		try {
			...
			$xml = $ws->delete($opt);
			$where = '`id_attribute`='.$id_atributo;
    ...
    public function escribirLog($text) {
        $path = $_SERVER['DOCUMENT_ROOT'] . '/modules/conector_bled/logs/log.log';
        $file = fopen($path, "a");
        fwrite($file, $text."\n");
        fclose($file);
    }
